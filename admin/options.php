...
function wsds_variation_product_options($loop, $variation_data, $variation) {
	...
	?>
   	<div class ='wsds_options_panel wsds_variation_field_group <?php if($status!=1){ esc_attr_e("wsds-disable-status"); } ?>'>
		<p class='form-field wsds_select_status'>
			<label for='wsds_select_status'><?php _e( 'Schedule Sale Discount Status', 'woocommerce-sale-discount-scheduler' ); ?></label>
		...
		<p class="form-field wsds_show_countdown wsds_dependent_parent">
			<label for='wsds_enable_countdown_on_single_product'><?php _e( 'Single product Countdown', 'woocommerce-sale-discount-scheduler' ); ?></label>
			<input type="checkbox" name="wsds_enable_countdown_on_single_product[<?php esc_attr_e($variation_id); ?>]" id="wsds_enable_countdown_on_single_product_<?php esc_attr_e($variation_id); ?>" value="on" <?php checked( $wsds_single_product_countdown, 'on' ); ?> />
			<span for="wsds_enable_countdown_on_single_product">Enable</span><br>
			<span class="wsds_note"><?php _e('Enable this to display countdown in single product page.','woocommerce-sale-discount-scheduler'); ?></span>
		</p>
		<p class="form-field wsds_note wsds_dependent_parent"><?php echo esc_html__('Note: Start time and End time will be on GMT, Current GMT time is:','woocommerce-sale-discount-scheduler'); ?> <?php echo date("Y-m-d @ H:i",$now); ?><br> Note: Sale price discount will be consider on regular price.  </p>
		<div class="wsds_save-variation">				
			<button type="button" class="button-primary save-variation-changes">Save changes</button><div class="clear"></div>
		</div>
    </div><?php
}
...
function wsds_save_variation_product_options($post_id) {
	...
	if(isset($_POST['wsds_enable_countdown_on_single_product'][$post_id])) $wsds_single_product_countdown=sanitize_text_field($_POST['wsds_enable_countdown_on_single_product'][$post_id]);
